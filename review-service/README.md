...
Ce dépôt contient désormais une structure permettant d'extraire un microservice dédié **review**. Les points clés :

- Entrypoint : `com.littlebook.review.ReviewApplication` (scan limité au package `com.littlebook.admin`).
- Port par défaut : `8082` (fichier `src/main/resources/application.yml`).
- Endpoints d'exemple : `/admin/health`, `/review/ping`.
...
```bash
 # admin-service — microservice review

 Ce dossier contient le microservice "review" extrait du backend LittleBook.
 ...
 - Entrypoint : `com.littlebook.admin.ReviewApplication`
 - Port par défaut : 8082
 - Endpoints d'exemple : `/review/health`, `/review/ping`

 Important : ce README couvre uniquement le microservice `review-service`. Les sources plus larges du backend (monolithe) ont été archivées ou déplacées — voir `archived/` si présent.

 ## Structure du projet

 - `src/main/java/com/littlebook/review` : code spécifique au microservice (controller, service, repository, config, dto)
 ...
 ```bash
 cd /Users/justinekosinski/Desktop/ledoux/LittleBook_Back/review-service
 mvn -DskipTests clean package
 ```

 2) Lancer avec Maven (utilise l'entrypoint review) :

 ```bash
 mvn -DskipTests -Dspring-boot.run.main-class=com.littlebook.review.ReviewApplication spring-boot:run
 ```
 ...
 ```bash
 curl -sS http://localhost:8082/review/health
 curl -sS http://localhost:8082/review/ping
 ```

 Si vous utilisez un port autre que 8082, passez l'argument `--server.port=XXXX` à la JVM ou à `spring-boot:run`.
 ...
 ```bash
 # depuis le dossier review-service
 docker build -t littlebook-review:local .
 docker run -p 8081:8081 littlebook-review:local
 ```
